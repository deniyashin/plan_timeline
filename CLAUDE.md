...
Статический сайт на GitHub Pages. Таймлайн программ и проектов медицинской компании: 5 направлений изменений, 38 программ, 212 проектов, сроки по месяцам 2026–2027.

**Нет сборщика (webpack/vite)** — скрипты грузятся через `<script>` теги. Деплой: push в `main` → GitHub Actions → GitHub Pages.

## Стек

- Vanilla JS (IIFE-модули, глобалы через `window.*`)
- HTML5 + CSS3 (нет фреймворков)
- n8n webhook для сохранения/загрузки данных
...
| Публикация (n8n webhook) | `js/modules/publish.js` |
...
- При публикации: собирается состояние из DOM + localStorage → отправляется на n8n webhook → сохраняется обратно в `data.json`
